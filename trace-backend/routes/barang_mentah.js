...
router.get('/:id', verifyToken, (req, res) => {
    ...
    db.query('SELECT * FROM Barang_Mentah WHERE barang_mentah_id = ?', [id], (err, results) => {
        ...
        if (results.length === 0) {
            return res.status(404).json({ error: true, message: 'Barang Mentah not found' });
        }
        const result = results[0];
        res.json({ error: false, message: 'success', result: { materialId: `material-${result.barang_mentah_id}`, name: result.nama_barang_mentah, type: result.jenis_barang_mentah, description: result.deskripsi_barang_mentah, image: result.image, price: result.price, supplierId: `user-${result.supplier_id}` } });
    });
});
